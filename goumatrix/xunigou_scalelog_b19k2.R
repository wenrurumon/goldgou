...
k <- 2
testperiod <- length(datapool)-(length(datapool)-20):0
...
bench %>%
  group_by('total') %>%
  summarise(min(obs),max(sell),prod(bench))
...
# save(tests,file='trace_scalelog_0921b19k2.rda')

valid <- sapply(5:40,function(i){
  rbindlist(lapply(tests,head,i)) %>%
    group_by(obs) %>%
    summarise(actual=mean(actual)) %>%
    merge(bench) %>%
    summarise(prod(actual),prod(bench),mean(actual>bench))
})
...
rbindlist(lapply(tests,head,5)) %>%
  ...
  summarise(actual=prod(actual,na.rm=T)) %>%
  merge(
    bench %>%
      group_by(obs=substr(obs,1,6)) %>%
      summarise(bench=prod(bench))
  ...
  melt(id=1) %>%
  mutate(obs=match(obs,sort(unique(substr(rawhis$date,1,6))))) %>%
  ggplot() + 
