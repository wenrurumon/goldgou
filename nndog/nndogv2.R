...
# system.time(datasets <- getmodelfile_trian2(raw=raw,ni,nj))
...
system.time(
  test <- lapply(1:(length(datasets)-ob.days2-1),function(i){
    print(paste(i,Sys.time()))
    #setup dataset
    dataseti.train <- datasets[1:ob.days2+i-1]
    dataseti.obs <- datasets[ob.days2+nj+i-1]
    dataseti.train <- lapply(dataseti.train,function(x){
      xi <- log(x[[1]][,-1])
      na2 <- max(xi,na.rm=T)*2
      inf2 <- min(xi[xi!=-Inf],na.rm=T)*2
    ...
    pool <- data.table(
      buy=unique(Ytr$buy),
      dist=colMeans((t(Zidxtr)-Zidxte)^2),
      sign=colMeans(t(Ztr-1)*(Zte[1,]-1)>0)
    ) %>%
      arrange(dist)
    #Train and Test set
    # dataseti.train2 <- dataseti.train[which(sapply(dataseti.train,function(x){x[[2]]$buy[1]})%in%pool)]
    dataseti.train2 <- dataseti.train
    Xtr2 <- do.call(rbind,lapply(dataseti.train2,function(x){x[[1]]})) %>% t
    Ytr2 <- do.call(rbind,lapply(dataseti.train2,function(x){x[[2]]}))
    Xte <- do.call(rbind,lapply(dataseti.obs,function(x){x[[1]]})) %>% t
    Yte <- do.call(rbind,lapply(dataseti.obs,function(x){x[[2]]}))
    #KNN
    # w <- pool$sign[match(Ytr$buy,pool$buy)]
    w <- 1
    system.time(
      outi <- data.table(
        Yte,
