...
# rawsina %>%
#   arrange(date,code) %>%
#   filter(date>=20250601) %>%
#   group_by(code) %>%
#   summarise(
#     open=open[1],close=close[length(close)]
#   ) %>%
#   mutate(gr=close/open-1) %>%
#   arrange(desc(gr)) %>%
#   write.csv('growth_since_Jun25.csv')
...
# stock_list <- stock_list_sina %>%
#   select(code=`代码`,name=`名称`,
#          new=`最新价`,close1=`昨收`,open0=`今开`,low0=`最低`,high0=`最高`,
#          value=`成交额`,amount=`成交额`) %>% mutate(code=as.numeric(substr(code,3,8)))
...
list0 <- function(){
  ...
  openinfo <- ak$stock_zh_a_spot_em()
  ...
  if(as.numeric(gsub('-','',Sys.Date())) %in% obsmap$buy){ #今天是交易日
    ...
    onite <- openinfo %>%
      ...
      mutate(code=ifelse(nchar(code)>6,as.numeric(substr(code,3,8)),code)) %>%
  ...
  X <- X %>%
    ...
    mutate(
      ztrate2=(high1/close2>=ztrate)+0,
      ztrate3=(low1/close2>=ztrate)+0,
      ztrate=ztrate,
      open1=open1/open0,
      close1=close1/open0,
      high1=high1/open0,
    ...
    select(obs,code,open1,onite0,high1,low1,jg1,jg2,jg35,jg10,ojg1,closegr,ztrate,ztrate2,ztrate3,btoday1,bonite0,broi1,broi5,class) 
...
buy2test <- (obsmap %>% filter(obs>=20240700,sell%in%rownames(close)))$buy

filename <- 'test'
set.seed(1); seeds <- sample(1:10000,500) #设定数据随机数
sample.rate <- 0.1

system.time(
  
  tests <- lapply(buy2test,function(buyi){
    
    print(paste(Sys.time(),buyi))
    ...
    mfiles.test <- lapply(unique(datasets$class),function(i){
      datai <- datasets %>%
        ...
        select(buy,code,open1,onite0,high1,low1,jg1,jg2,jg35,jg10,ojg1,closegr,y,z,
               ztrate,ztrate2,ztrate3,btoday1,bonite0,broi1,broi5,class) 
    ...
    mfiles.train <- lapply(unique(datasets$class),function(i){
      datai <- datasets %>%
        filter(class==i) %>%
        filter(jg10>0) %>%
        filter(buy<(obsmap %>% filter(sell==buyi))$obs) %>%
        mutate(
          ztrate2=(high1/close2>=ztrate)+0,
          ztrate3=(low1/close2>=ztrate)+0,
          ztrate=ztrate,
    ...
    mfiles.train <- mfiles.train[which(sapply(mfiles.test,nrow)>0)]
    mfiles.test <- mfiles.test[which(sapply(mfiles.test,nrow)>0)]
    mfile <<- mfiles.train[[1]]
    ...
    clusterExport(cl, varlist = c("mfile", "seeds", "sample.rate"))
    clusterEvalQ(cl, {
      library(rpart)
      library(dplyr)
    })
    ...
    system.time(
      models <- parLapply(cl,1:length(seeds),function(i){
        set.seed(seeds[i])
    ...
    models_y <- lapply(models,function(x){x[[1]]})
    models_z <- lapply(models,function(x){x[[2]]})
    pred.y <- sapply(models_y,predict,mfiles.test[[1]])
    pred.z <- sapply(models_z,predict,mfiles.test[[1]])
    
    out <- data.table(
      mfiles.test[[1]] %>%
        mutate(jg0=ojg1/jg10) %>%
        select(buy,code,onite0,y,jg0),
      modelmax=max(mfile$buy),
      py=rowMeans(pred.y),wy=rowMeans(pred.y>0),pz=rowMeans(pred.z),wz=rowMeans(pred.z>0)
    ) %>%
...
temp <- rbindlist(tests) %>%
  ...
  merge(
    datasets %>% group_by(buy) %>% summarise(bench0=mean(roi)),all.x=T
  ) %>%
  mutate(roi=ifelse(is.na(roi),1,roi)) 

temp$cumbench0 <- cumprod(temp$bench0)
temp$cumbench <- cumprod(temp$bench)
temp$cumroi <- cumprod(temp$roi)

temp %>% 
  select(buy,cumbench,cumroi,cumbench0) %>%
  melt(id=1) %>%
  ggplot() + 
  geom_line(aes(x=as.Date(paste(substr(buy,1,4),substr(buy,5,6),substr(buy,7,8),sep='-')),y=value,colour=variable))
