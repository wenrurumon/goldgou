...
if(note=='test'):
  def printlog(x):
    ...
    logging.basicConfig(
        ...
        filename=f'log/testdg1_{note}.log',  
        filemode='a'  
    )
...
def testdata(arg1,prd1=40):
    raw = RAW[RAW['date']<=datekey[arg1]]
...
def voting2(votes,prop_votes,prop_robots,prop_codes,w):
    ...
    rlt['codes'] = rlt.index
    rlt['date'] = arg1
    rlt['idx'] = rlt['count']/(num_robots)/(votes.shape[2]/len(codes))
    rlt = rlt[rlt['idx']>=np.quantile(rlt['idx'],prop_codes)]
    rlt['share'] = rlt['count']/sum(rlt['count'])
    return(rlt)
...
RAW = pd.read_csv(f'data/rawtest2022.csv')
RAW['date'] = pd.to_datetime(RAW['date'])
...
for datei in range(24):
    ...
    models = np.load(f"rlt/tvote_{arg1}.npz",allow_pickle=True)
    votes = models['votes']
    rlt = voting2(votes,prop_votes,prop_robots,prop_codes,w)
