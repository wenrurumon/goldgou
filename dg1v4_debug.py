...
np.savez(f'{rltfolder}/vote{date0}.npz',votes=votes,raw=raw)
...
for datai in range(2,len(codelist)):
    ...
    rlt['date'] = date1
    printlog(pd.merge(rlt,ak.stock_info_a_code_name(),left_index=True, right_on='code'))
    trans.append(rlt)
    #Calculation
    transi = rlt
    refi = []
